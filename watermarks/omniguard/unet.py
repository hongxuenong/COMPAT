...
class Unet1(nn.Module):
    def __init__(self, resolution=256, secret_len=100, width=32, ndown=4, nmiddle=1, activ='silu'):
        super().__init__()
...
class SecretDecoder(nn.Module):
    def __init__(self, arch='resnet50', resolution=224, secret_len=100, pretrained=True):
        super().__init__()
        self.resolution = resolution
        self.arch = arch
        # kornia returns the same result as torch.nn.functional but faster
        self.preprocess = kornia.augmentation.Resize(size=(resolution, resolution), align_corners=False)

        if pretrained:
            print('[Decoder] adopt pretrained decoder!')

        if arch == 'resnet50':
            if pretrained:
                weights = torchvision.models.ResNet50_Weights.IMAGENET1K_V2
            self.decoder = torchvision.models.resnet50(weights=weights)
            # self.decoder = timm.create_model('resnet50.a1_in1k', pretrained=pretrained)
            self.decoder.fc = nn.Linear(self.decoder.fc.in_features, secret_len)
        else:
            raise ValueError('Unknown architecture')

    def forward(self, image, **kwargs):
        if min(image.shape[-2:]) > self.resolution:
