...
class PromptGenBlock(nn.Module):
    def __init__(self, prompt_dim=768, prompt_len=6, lin_dim = 768, prompt_size=16):
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        self.promptblock = PromptGenBlock()
    ...
    def forward(self, x, container):
        container = container.to(torch.float32)
        x = self.patch_embed(x)
        container = self.patch_embed(container)
        prompt = self.promptblock(container)
        x = self.delta * prompt + x
