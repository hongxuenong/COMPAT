...
class INV_block_transform(nn.Module):
    def __init__(self, clamp=c.clamp, harr=True, in_1=3, in_2=3):
        super().__init__()
        if harr:
            self.split_len1 = in_1 * 4
            self.split_len2 = in_2 * 4
        self.clamp = clamp
        # ρ
        self.r = nn.Conv2d(self.split_len1, self.split_len2, 3, padding=1)
        # η
        self.y = nn.Conv2d(self.split_len1, self.split_len2, 3, padding=1)
        # φ
        self.f = nn.Conv2d(self.split_len2, self.split_len1, 3, padding=1)
